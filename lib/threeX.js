...
THREEx.DayNight.StarField = function() {
    // create the mesh
    var texture = THREE.ImageUtils.loadTexture(THREEx.DayNight.baseURL + 'assets/images/galaxy_starfield.png')
...
THREEx.DayNight.SunLight = function() {
    ...
    this.update = function(sunAngle) {
        light.position.x = Math.cos(sunAngle) * 200;
        light.position.y = Math.sin(sunAngle) * 400;
        light.position.z = Math.cos(sunAngle) * 200;
...
THREEx.DayNight.SunSphere = function() {
    ...
    this.update = function(sunAngle) {
        mesh.position.x = Math.cos(sunAngle) * 200;
        mesh.position.y = Math.sin(sunAngle) * 400;
        mesh.position.z = Math.cos(sunAngle) * 200;

        var phase = THREEx.DayNight.currentPhase(sunAngle)
        if (phase === 'day') {
            mesh.material.color.set("rgb(255,255,0)");
        } else if (phase === 'twilight') {
            mesh.material.color.set("rgb(255,255,0)");
        } else {}
    }
